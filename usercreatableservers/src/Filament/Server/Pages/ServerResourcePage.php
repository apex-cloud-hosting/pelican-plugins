...
class ServerResourcePage extends ServerFormPage
{
    ...
    {
        if (!config('usercreatableservers.can_users_update_servers')) {
            return false;
        }
        ...
        return $form
            ...
            ->schema([
                TextInput::make('cpu')
                    ...
                    ->required()
                    ->live(onBlur: true)
                    ->hint(fn ($state) => $userResourceLimits->cpu > 0 ? ($maxCpu - $state . '% ' . trans('usercreatableservers::strings.left')) : trans('usercreatableservers::strings.unlimited'))
                ...
                TextInput::make('memory')
                    ...
                    ->required()
                    ->live(onBlur: true)
                    ->hint(fn ($state) => $userResourceLimits->memory > 0 ? ($maxMemory - $state . $suffix . ' ' . trans('usercreatableservers::strings.left')) : trans('usercreatableservers::strings.unlimited'))
                ...
                TextInput::make('disk')
                    ...
                    ->required()
                    ->live(onBlur: true)
                    ->hint(fn ($state) => $userResourceLimits->disk > 0 ? ($maxDisk - $state . $suffix . ' ' . trans('usercreatableservers::strings.left')) : trans('usercreatableservers::strings.unlimited'))
